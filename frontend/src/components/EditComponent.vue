  ...
  <div>
    <form novalidate class="md-layout" @submit.prevent="validateVehicle">
      <md-card class="md-layout-item md-size-100 md-small-size-100">
        ...
        <md-card-actions>
            <md-button class="md-accent"  v-on:click="emmitClose(-1)">Fechar</md-button>
            <md-button class="md-accent" v-on:click="deleteVehicle()">Deletar</md-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      deleteVehicle(){
          this.sending = true
          axios.delete(`http://localhost:8000/api/vehicle/${this.form.id}/`).then(response=>{
              response
              this.sending = false
              this.emmitClose(this.form.id)
      ...
      saveVehicle () {
        this.sending = true
        axios.put(`http://localhost:8000/api/vehicle/${this.form.id}/`, this.form,
        {crossdomain: true})
        .then(response=>{
          response
